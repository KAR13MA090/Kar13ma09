...
def get_http_proxies() -> List[Dict[str, str]]:
    ...
    try:
        ...
        print(
            f"\n{F.RED}[!] {F.CYAN}Không thể kết nối với các proxy.!{F.RESET}"
        )
        sys.exit(1)
    except ConnectionError:
        print(f"\n{F.RED}[!] {F.CYAN}Thiết bị không được kết nối với Internet.!{F.RESET}")
        sys.exit(1)
...
def flood(target: str) -> None:
    ...
    try:
        ...
        payload_size = f"{F.RESET} Kích thước dữ liệu yêu cầu: {F.CYAN}{round(len(response.content)/1024, 2):>6} KB"
