...
class AttackMethod:
    ...
    def __run_threads(self) -> None:
        ...
        print(f"{F.MAGENTA}\n\n[!] {F.BLUE}Cuộc tấn công đã hoàn tất!\n\n{F.RESET}")
    ...
    def start(self) -> None:
        ...
        print(
            f"{F.MAGENTA}\n[!] {F.BLUE}Tấn công {F.MAGENTA}{self.target} {F.BLUE}"
            f"using {F.MAGENTA}{self.method_name.upper()}{F.BLUE} method {F.MAGENTA}\n\n"
            f"[!] {F.BLUE}Cuộc tấn công sẽ dừng lại sau khi {F.MAGENTA}{duration}{F.BLUE}\n{F.RESET}"
        )
        if "slowloris" in self.method_name:
            print(
                f"{F.MAGENTA}[!] {F.BLUE}Các socket đã ngừng hoạt động sẽ được tự động tái tạo!\n\n{F.RESET}"
            )
        elif self.method_name == "http-proxy":
            print(
                f"{F.MAGENTA}[!] {F.BLUE}Các proxy không trả về trạng thái 200 sẽ tự động được thay thế!\n\n{F.RESET}"
            )
        ...
        try:
            ...
            self.is_running = False
            print(
                f"{F.RED}\n\n[!] {F.MAGENTA}Ctrl+C Để dừng tấn công...{F.RESET}"
            )
            ...
            print(f"{F.MAGENTA}\n[!] {F.BLUE}Cuộc tấn công bị gián đoạn!\n\n{F.RESET}")
            sys.exit(1)
