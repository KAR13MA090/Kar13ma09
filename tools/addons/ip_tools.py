    ...
    try:
        ...
        for ip in ipv4:
            if ipaddress.ip_address(origin) in ipaddress.ip_network(ip):
                print(
                    f"\n{F.RED}[!] {F.CYAN}Trang web này được bảo vệ bởi CloudFlare, cuộc tấn công này có thể không đạt được kết quả mong muốn...{F.RESET}"
                )
                sleep(1)
    except (Timeout, socket.gaierror):
        print(
            f"{F.RED}\n[!] {F.CYAN}Không thể kiểm tra bảo vệ CloudFlare!.{F.RESET}"
        )
        sleep(1)
...
def get_host_ip() -> str:
    ...
    try:
        ...
        print(
            f"{F.YELLOW}│   └───{F.MAGENTA}[!] {F.BLUE}Không tìm thấy địa chỉ IP cục bộ!{F.RESET}"
        )
        sys.exit(1)
    s.close()
    return IP
...
def show_local_host_ips() -> None:
    ...
    print(f"{F.YELLOW}│   │")
    print(
        f"{F.YELLOW}│   ├───{F.MAGENTA} [!] {F.LIGHTCYAN_EX}Quét Mạng Cục Bộ...{F.RESET}"
    )
    print(f"{F.YELLOW}│   │")
    print(f"{F.YELLOW}│   ├───{F.BLUE} Các máy chủ có sẵn:{F.RESET}")
    print(f"{F.YELLOW}│   │")

    try:
        for host in __get_local_host_ips()[1:-1]:
            print(f"{F.YELLOW}│   │    {F.GREEN} {host}{F.RESET}")
        print(f"{F.YELLOW}│   │")
    except IndexError:
        print(f"{F.YELLOW}│   ├───{F.MAGENTA} [!] {F.RED}Không có máy chủ nào khả dụng!{F.RESET}")
